...
@Tag(
        name = "CRUD REST APIs for Loans in SBank",
        description = "CRUD REST APIs in Sbank to CREATE, UPDATE, FETCH AND DELETE loan details"
)
@RestController
...
public class LoansController {
    ...
    @Operation(
            ...
            description = "REST API to create new loan inside SBank"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    public ResponseEntity<LoansDto> fetchLoanDetails(@RequestHeader("sbank-correlation-id") String correlationId,
       ...
       // logger.debug("sbank-correlation-id found: {}", correlationId);
        logger.debug("fetchLoanDetails method is starting");
